...
void Line_3D::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& commandList, Object_3D_Data& data) {
...
void Grid::Initialize(GameEngine* gameEngine) {
	...
	white2x2_ = new Texture;
	gameEngine->LoadTexture(white2x2_, "DebugResources/white2x2.png");
	lineX_->SetTexture(white2x2_);
	lineY_->SetTexture(white2x2_);
	lineZ_->SetTexture(white2x2_);
...
void Grid::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& commandList) {
	...
	if (xy_ && std::abs(CameraMatrix.m[3][2]) > 0.01f) {
		for (int i = 0; i <= kGridLength_ * 2; i++) {
			...
			if (data.transform.translate.y == 0.0f) {
				data.color = { 0.8f,0.0f,0.0f,1.0f };
				...
				data.color = { 0.5f,0.5f,0.5f,1.0f };
			...
			if (data.transform.translate.x == 0.0f) {
				data.color = { 0.0f,0.8f,0.0f,1.0f };
	...
	if (xz_ && std::abs(CameraMatrix.m[3][1]) > 0.01f) {
		for (int i = 0; i <= kGridLength_ * 2; i++) {
			...
			data.color = { 0.2f,0.2f,0.2f,1.0f };
			if (data.transform.translate.x == 0.0f) {
				data.color = { 0.0f,0.0f,0.8f,1.0f };
