...
void Line_3D::Initialize() {

	device_ = GameEngine::GetDevice();

	// 頂点リソースを作る
	vertexResource_ = CreateBufferResource(device_, sizeof(VertexData) * 2 * kMaxIndex_);
...
void Line_3D::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& commandList, const Segment& segment, const Vector4& color, Camera* camera) {
	...
	// 始点
	vertexData_[index_ * 2].position = { segment.origin.x, segment.origin.y, segment.origin.z, 1.0f };
	vertexData_[index_ * 2].texcoord = { 0.0f, 0.0f };
	vertexData_[index_ * 2].normal = { 0.0f, 0.0f, -1.0f };
	// 終点
	vertexData_[index_ * 2 + 1].position = { segment.origin.x + segment.diff.x, segment.origin.y + segment.diff.y, segment.origin.z + segment.diff.z, 1.0f };
	vertexData_[index_ * 2 + 1].texcoord = { 1.0f, 1.0f };
	vertexData_[index_ * 2 + 1].normal = { 0.0f, 0.0f, -1.0f };

	vertexResource_->Unmap(0, nullptr);

	// 頂点バッファビューを作成する
	// リソースの先頭のアドレスから使う
	vertexBufferView_[index_].BufferLocation = vertexResource_->GetGPUVirtualAddress() + index_ * sizeof(VertexData) * 2;
	// 使用するリソースのサイズは頂点は4つサイズ
	vertexBufferView_[index_].SizeInBytes = sizeof(VertexData) * 2;
	// 1頂点あたりのサイズ
	vertexBufferView_[index_].StrideInBytes = sizeof(VertexData);
...
void Grid::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& commandList) {
	...
	if (xy_ && std::abs(CameraMatrix.m[3][2]) > 0.01f) {
		for (int i = 0; i <= kGridLength_ * 2; i++) {
			segment.origin = { float(-kGridLength_ + centerPoint_.x),float(-kGridLength_ + centerPoint_.y) + i,0.0f };
			segment.diff = { float(kGridLength_ * 2), 0.0f, 0.0f };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.y == 0.0f) {
				color = { 0.8f,0.0f,0.0f,1.0f };
				segment.origin.z = 0.0004f;
			} else if (int(segment.origin.y) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.z = 0.0002f;
			} else if (int(segment.origin.y) % 10 == 0) {
				color = { 0.5f,0.5f,0.5f,1.0f };
				segment.origin.z = 0.0001f;
			}
			lineX_->Draw(commandList, segment, color, camera_);
			segment.origin = { float(-kGridLength_ + centerPoint_.x) + i, float(-kGridLength_ + centerPoint_.y), 0.0f };
			segment.diff = { 0.0f, float(kGridLength_ * 2), 0.0f };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.x == 0.0f) {
				color = { 0.0f,0.8f,0.0f,1.0f };
				segment.origin.z = 0.0003f;
			} else if (int(segment.origin.x) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.z = 0.0002f;
			} else if (int(segment.origin.x) % 10 == 0) {
				color = { 0.5f,0.5f,0.5f,1.0f };
				segment.origin.z = 0.0001f;
			}
			lineY_->Draw(commandList, segment, color, camera_);
		}
	}
	if (xz_ && std::abs(CameraMatrix.m[3][1]) > 0.01f) {
		for (int i = 0; i <= kGridLength_ * 2; i++) {
			segment.origin = { float(-kGridLength_ + centerPoint_.x), 0.0f, float(-kGridLength_ + centerPoint_.z) + i };
			segment.diff = { float(kGridLength_ * 2), 0.0f, 0.0f };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.z == 0.0f) {
				color = { 0.8f,0.0f,0.0f,1.0f };
				segment.origin.y = 0.0004f;
			} else if (int(segment.origin.z) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.y = 0.0002f;
			} else if (int(segment.origin.z) % 10 == 0) {
				color = { 0.5f,0.5f,0.5f,1.0f };
				segment.origin.y = 0.0001f;
			}
			lineX_->Draw(commandList, segment, color, camera_);
			segment.origin = { float(-kGridLength_ + centerPoint_.x) + i, 0.0f, float(-kGridLength_ + centerPoint_.z) };
			segment.diff = { 0.0f, 0.0f, float(kGridLength_ * 2) };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.x == 0.0f) {
				color = { 0.0f,0.0f,0.8f,1.0f };
				segment.origin.y = 0.0003f;
			} else if (int(segment.origin.x) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.y = 0.0002f;
			} else if (int(segment.origin.x) % 10 == 0) {
			...
			lineZ_->Draw(commandList, segment, color, camera_);
		}
	}
	if (yz_ && std::abs(CameraMatrix.m[3][0]) > 0.01f) {
		for (int i = 0; i <= kGridLength_ * 2; i++) {
			segment.origin = { 0.0f, float(-kGridLength_ + centerPoint_.y), float(-kGridLength_ + centerPoint_.z) + i };
			segment.diff = { 0.0f, float(kGridLength_ * 2), 0.0f };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.z == 0.0f) {
				color = { 0.0f,0.8f,0.0f,1.0f };
				segment.origin.x = 0.0004f;
			} else if (int(segment.origin.z) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.x = 0.0002f;
			} else if (int(segment.origin.z) % 10 == 0) {
				color = { 0.5f,0.5f,0.5f,1.0f };
				segment.origin.x = 0.0001f;
			}
			lineY_->Draw(commandList, segment, color, camera_);
			segment.origin = { 0.0f, float(-kGridLength_ + centerPoint_.y) + i, float(-kGridLength_ + centerPoint_.z) };
			segment.diff = { 0.0f, 0.0f, float(kGridLength_ * 2) };
			color = { 0.2f,0.2f,0.2f,1.0f };
			if (segment.origin.y == 0.0f) {
				color = { 0.0f,0.0f,0.8f,1.0f };
				segment.origin.x = 0.0003f;
			} else if (int(segment.origin.y) % 100 == 0) {
				color = { 1.0f,1.0f,1.0f,1.0f };
				segment.origin.x = 0.0002f;
			} else if (int(segment.origin.y) % 10 == 0) {
