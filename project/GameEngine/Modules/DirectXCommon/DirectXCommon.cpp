...
void DirectXCommon::RenderPreDraw(std::string textureName) {
	...
	int32_t rtvIndex = TextureManager::GetInstance()->GetRtvIndex(textureName);
	...
	barrier_.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;
	//バリアを張る対象のリソース。現在のバックバッファに対して行う
	barrier_.Transition.pResource = TextureManager::GetInstance()->GetResource(textureName);
	//遷移前(現在)のResourcesState
...
Microsoft::WRL::ComPtr <ID3D12RootSignature> DirectXCommon::Particle_RootSignatureInitialvalue() {
	...
	rootParameters[5].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;	//PixelShaderで使う
	rootParameters[5].DescriptorTable.pDescriptorRanges = depthDescriptorRange;	//Tableの中身の配列を指定
	rootParameters[5].DescriptorTable.NumDescriptorRanges = _countof(depthDescriptorRange);	//Tableで利用する数
	descriptionRootSignature.pParameters = rootParameters;	//ルートパラメータ配列へのポインタ
	descriptionRootSignature.NumParameters = _countof(rootParameters);	//配列の長さ
...
Microsoft::WRL::ComPtr <ID3D12RootSignature> DirectXCommon::Screen_Outline_RootSignatureInitialvalue() {
	...
	rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;	//DescriptorTableを使う
	rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;	//PixelShaderで使う
	rootParameters[2].DescriptorTable.pDescriptorRanges = depthDescriptorRange;	//Tableの中身の配列を指定
	rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(depthDescriptorRange);	//Tableで利用する数
	descriptionRootSignature.pParameters = rootParameters;	//ルートパラメータ配列へのポインタ
	descriptionRootSignature.NumParameters = _countof(rootParameters);	//配列の長さ
	...
	staticSamplers[1].ShaderRegister = 1;	//レジスター番号1を使う
...
Microsoft::WRL::ComPtr <ID3D12RootSignature> DirectXCommon::Screen_Dissolve_RootSignatureInitialvalue() {
	...
	D3D12_DESCRIPTOR_RANGE maskDescriptorRange[1] = {};
	maskDescriptorRange[0].BaseShaderRegister = 1;	//1から始める
	maskDescriptorRange[0].NumDescriptors = 1;	//数は1つ
	maskDescriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;	//SRVを使う
	maskDescriptorRange[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;	//Offsetを自動計算
	...
	rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;	//DescriptorTableを使う
	rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;	//PixelShaderで使う
	rootParameters[2].DescriptorTable.pDescriptorRanges = maskDescriptorRange;	//Tableの中身の配列を指定
	rootParameters[2].DescriptorTable.NumDescriptorRanges = _countof(maskDescriptorRange);	//Tableで利用する数
	descriptionRootSignature.pParameters = rootParameters;	//ルートパラメータ配列へのポインタ
	descriptionRootSignature.NumParameters = _countof(rootParameters);	//配列の長さ
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::Compute_Skinning_RootSignatureInitialvalue() {
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::Compute_Initialize_Particle_RootSignatureInitialvalue() {
	D3D12_DESCRIPTOR_RANGE descriptorRange[1] = {};
	descriptorRange[0].BaseShaderRegister = 0;	//0から始める
	descriptorRange[0].NumDescriptors = 1;	//数は1つ
	...
	freeListIndexDescriptorRange[0].BaseShaderRegister = 1;	//1から始める
	freeListIndexDescriptorRange[0].NumDescriptors = 1;	//数は1つ
	...
	freeListDescriptorRange[0].BaseShaderRegister = 2;	//1から始める
	freeListDescriptorRange[0].NumDescriptors = 1;	//数は1つ
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::Compute_Emit_Particle_RootSignatureInitialvalue() {
	D3D12_DESCRIPTOR_RANGE descriptorRange[1] = {};
	descriptorRange[0].BaseShaderRegister = 0;	//0から始める
	descriptorRange[0].NumDescriptors = 1;	//数は1つ
	descriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;	//UAVを使う
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::Compute_Update_Particle_RootSignatureInitialvalue() {
	D3D12_DESCRIPTOR_RANGE descriptorRange[1] = {};
	descriptorRange[0].BaseShaderRegister = 0;	//0から始める
	descriptorRange[0].NumDescriptors = 1;	//数は1つ
	descriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;	//UAVを使う
...
DirectXCommon::RTVResource DirectXCommon::CreateRenderTextureResource(uint32_t width, uint32_t height, DXGI_FORMAT format, const Vector4 clearColor) {
	...
	HRESULT hr = device_->CreateCommittedResource(
		...
		&clearValue,	//Clear最適値。
		IID_PPV_ARGS(&rtvResource.resource)	//作成するResourceポインタへのポインタ
	);
	assert(SUCCEEDED(hr));

	//レンダーテクスチャを最大数以上は作らせない
	assert(renderDescriptorIndex_ < kOffscreenDescriptorSize_);
	//RTVハンドルを取得。スワップチェーンで使っているものの次を使う
	rtvResource.rtvIndex = kSwapChainDescriptorSize_ + renderDescriptorIndex_;
	renderDescriptorIndex_++;

	//RTVハンドルを取得。スワップチェーンで使っているものの次を使う
	rtvHandles_[rtvResource.rtvIndex] = GetCPUDescriptorHandle(rtvDescriptorHeap_, descriptorSizeRTV_, rtvResource.rtvIndex);
...
void DirectXCommon::UpdateDepthTexture() {
...
void DirectXCommon::descriptorHeapInitialize() {
	...
	//RTV用のヒープでディスクリプタ数は3。RTVはShader内で触る物ではないので、ShaderVisibleはfalse
